...
/**
  ...
  */
__weak int16_t SPD_GetElAngle(const SpeednPosFdbk_Handle_t *pHandle) //它返回最后计算的转子电角度
{
...
/**
  ...
  */
__weak int32_t SPD_GetMecAngle(const SpeednPosFdbk_Handle_t *pHandle) //它返回最后计算的转子机械角度
{
#ifdef NULL_PTR_SPD_POS_FBK
...
/**
  ...
  * @retval none
  */
__weak bool SPD_IsMecSpeedReliable(SpeednPosFdbk_Handle_t *pHandle, const int16_t *pMecSpeedUnit)  //速度、加速度是否在可靠范围内
{
  bool SpeedSensorReliability = true;
#ifdef NULL_PTR_SPD_POS_FBK
  ...
  else
  {
    ...
    if (hAbsMecSpeedUnit > pHandle->hMaxReliableMecSpeedUnit)  //速度大/小于最大/小设定 ---- SpeedError
    {
      SpeedError = true;
    }
    ...
    if (hAbsMecAccelUnitP > pHandle->hMaxReliableMecAccelUnitP)  //加速度大于最大 ---- SpeedError
    {
      SpeedError = true;
    }
    ...
    if (bSpeedErrorNumber == bMaximumSpeedErrorsNumber)
    {
      SpeedSensorReliability = false;        //速度传感器不可靠
    }
