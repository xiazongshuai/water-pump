...
/**
  ...
  */
__weak int16_t STO_PLL_CalcElAngle(STO_PLL_Handle_t *pHandle, Observer_Inputs_t *pInputs) //计算估计的电角度
{
  int16_t retValue;
  ...
  else
  {
    ...
    else if (pHandle->wBemf_alfa_est <= (-INT16_MAX * ((int32_t)pHandle->hF2))) //F2---4096
    {
      pHandle->wBemf_alfa_est = -INT16_MAX * ((int32_t)pHandle->hF2);
    }
    else
    ...
    //cstat !MISRAC2012-Rule-1.3_n !ATH-shift-neg !MISRAC2012-Rule-10.1_R6
      hAux_Alfa = (int16_t)(pHandle->wBemf_alfa_est >> pHandle->F2LOG);  //LOG2((F2))  //反电动势Alfa
#else
...
#ifndef FULL_MISRA_C_COMPLIANCY_STO_PLL
    //cstat !MISRAC2012-Rule-1.3_n !ATH-shift-neg !MISRAC2012-Rule-10.1_R6
    hAux_Beta = (int16_t)(pHandle->wBemf_beta_est >> pHandle->F2LOG);  //反电动势Beta
#else
    hAux_Beta = (int16_t)(pHandle->wBemf_beta_est / pHandle->hF2);
#endif

    if (pHandle->Ialfa_est > (INT16_MAX * ((int32_t)pHandle->hF1)))  //F1--16384
    {
...
#ifndef FULL_MISRA_C_COMPLIANCY_STO_PLL
    //cstat !MISRAC2012-Rule-1.3_n !ATH-shift-neg !MISRAC2012-Rule-10.1_R6
    hIalfa_err = (int16_t)(pHandle->Ialfa_est >> pHandle->F1LOG); //LOG2((F1))
#else
    ...
    wAux = ((int32_t)pHandle->hC1) * hAux;   // C1 (int32_t)((((int16_t)F1)*RS)/(LS*TF_REGULATION_RATE))
    wIalfa_est_Next = pHandle->Ialfa_est - wAux;

    wAux = ((int32_t)pHandle->hC2) * hIalfa_err;  //C2 (int32_t) GAIN1
    ...
    if (0 == pHandle->hForcedDirection)  //方向设置
    {
    ...
    pHandle->_Super.InstantaneousElSpeedDpp = hRotor_Speed;  //瞬时计算出的电速度，单位为[dpp]

    STO_Store_Rotor_Speed(pHandle, hRotor_Speed); //存速度(角度？？转速积分求取角度)
...
/**
  ...
  */
__weak bool STO_PLL_CalcAvrgMecSpeedUnit(STO_PLL_Handle_t *pHandle, int16_t *pMecSpeedUnit)  //计算平均机械速度
{
  bool bAux;
#ifdef NULL_PTR_STO_PLL_SPD_POS_FDB
  ...
  else
  {
    ...
    uint8_t i, bSpeedBufferSizeUnit = pHandle->SpeedBufferSizeUnit; //64 平均估计速度的 FIFO

    for (i = 0U; i < bSpeedBufferSizeUnit; i++)
    {
      wAvrSpeed_dpp += (int32_t)(pHandle->Speed_Buffer[i]);  //Speed_Buffer 为STO_PLL_CalcElAngle计算的速度
    }
    ...
    {
      wAvrSpeed_dpp = wAvrSpeed_dpp / ((int16_t)bSpeedBufferSizeUnit);  //取平均
    }
    ...
    for (i = 0U; i < bSpeedBufferSizeUnit; i++)
    {
      ...
      wAvrQuadraticError += wError;   //累加误差的平方
    }

    /* It computes the measurement variance */   //方差
    wAvrQuadraticError = wAvrQuadraticError / ((int16_t)bSpeedBufferSizeUnit); //误差平方的平均
    ...
    wAvrSquareSpeed = lAvrSquareSpeed / (int16_t)128;  //VariancePercentage乘了128，这里除128
    
    if (wAvrQuadraticError < wAvrSquareSpeed)
    {
      bIs_Speed_Reliable = true;       //方差小于设定值，速度可靠(一段数据内，速度波动不大就可靠？？)
    }
		

    /* Computation of Mechanical speed Unit */  //机械速度单位的计算
    wAux = wAvrSpeed_dpp * ((int32_t)pHandle->_Super.hMeasurementFrequency);  //平均速度*pwm频率
    wAux = wAux * ((int32_t)pHandle->_Super.SpeedUnit);  // U_01HZ 10
    wAux = wAux / ((int32_t)pHandle->_Super.DPPConvFactor);
    wAux = wAux / ((int16_t)pHandle->_Super.bElToMecRatio); //电角速度转机械速度

    *pMecSpeedUnit = (int16_t)wAux;
    pHandle->_Super.hAvrMecSpeedUnit = (int16_t)wAux;  //平均机械速度

    pHandle->IsSpeedReliable = bIs_Speed_Reliable;

    /* Bemf Consistency Check algorithm */
    if (true == pHandle->EnableDualCheck) /*do algorithm if it's enabled*/ //根据观察到的Bemf启用额外的可靠性检查
    {
      /* wAux abs value   */
    ...
    /* Decision making */  //决策
    if (false == pHandle->IsAlgorithmConverged) //观测器收敛
    {
...
/**
  ...
  * @param  pHandle: Handler of the current instance of the STO component.
  */
__weak void STO_PLL_CalcAvrgElSpeedDpp(STO_PLL_Handle_t *pHandle)  //计算电角度速度
{
#ifdef NULL_PTR_STO_PLL_SPD_POS_FDB
  if (MC_NULL == pHandle)
  {
  ...
  else
  {
    ...
    {
      wSum = wSum + pHandle->Speed_Buffer[hIndexNew] - pHandle->SpeedBufferOldestEl;  //最新值减上一次的值做累加
    }
    else
    {
...
#ifndef FULL_MISRA_C_COMPLIANCY_STO_PLL
  //cstat !MISRAC2012-Rule-1.3_n !ATH-shift-neg !MISRAC2012-Rule-10.1_R6
  wAvrSpeed_dpp = wSum >> pHandle->SpeedBufferSizeDppLOG;  //求平均
#else
...
/**
  ...
  * 
  */
inline static void STO_Store_Rotor_Speed(STO_PLL_Handle_t *pHandle, int16_t hRotor_Speed)  //存储速度
{
...
/**
  * @brief  Executes PLL algorithm for rotor position extraction from B-emf alpha and beta.   //提取转子位置
  * 
  * @param  pHandle: Handler of the current instance of the STO component.
  ...
  * @retval 
  */
inline static int16_t STO_ExecutePLL(STO_PLL_Handle_t *pHandle, int16_t hBemf_alfa_est, int16_t hBemf_beta_est) //定子参考系上估计的 alpha  beta
{
  ...
  Local_Components = MCM_Trig_Functions(pHandle->_Super.hElAngle); //计算sin  cos
...
/**
  ...
  */
__weak bool STO_PLL_IsObserverConverged(STO_PLL_Handle_t *pHandle, int16_t *phForcedMecSpeedUnit)  //检查状态观测器算法是否收敛。
{
  bool bAux = false;
#ifdef NULL_PTR_STO_PLL_SPD_POS_FDB
  ...
  else
  {
    ...
    else  //执行else
    {
      ...
      if (wtemp > 0)
      {
        ...
        if (true == pHandle->IsSpeedReliable)   //速度可靠(方差足够小)
        {
          if ((uint16_t)hEstimatedSpeedUnit > pHandle->MinStartUpValidSpeed)  //大于启动最小速度
          {
...
/**
  ...
  */
__weak alphabeta_t STO_PLL_GetEstimatedBemf(STO_PLL_Handle_t *pHandle) //导出估计的反电动势 alpha-beta
{
  alphabeta_t vaux;
#ifdef NULL_PTR_STO_PLL_SPD_POS_FDB
  if (MC_NULL == pHandle)
...
/**
  ...
  */
__weak alphabeta_t STO_PLL_GetEstimatedCurrent(STO_PLL_Handle_t *pHandle)  //状态观测器估计的定子电流α-β
{
  alphabeta_t iaux;
#ifdef NULL_PTR_STO_PLL_SPD_POS_FDB
  if (MC_NULL == pHandle)
...
/**
  ...
  */
__weak bool STO_PLL_IsVarianceTight(const STO_Handle_t *pHandle) //速度是否可靠
{
  bool tempStatus;
#ifdef NULL_PTR_STO_PLL_SPD_POS_FDB
  if (MC_NULL == pHandle)
...
/**
  ...
  * @param  pHandle: Handler of the current instance of the STO component.
  */
__weak void STO_PLL_ForceConvergency1(STO_Handle_t *pHandle) //状态观测器强制收敛
{
#ifdef NULL_PTR_STO_PLL_SPD_POS_FDB
  if (MC_NULL == pHandle)
  {
...
/**
  ...
  * @param  pHandle: Handler of the current instance of the STO component.
  */
__weak void STO_PLL_ForceConvergency2(STO_Handle_t *pHandle) //状态观测器强制收敛
{
#ifdef NULL_PTR_STO_PLL_SPD_POS_FDB
  if (MC_NULL == pHandle)
  {
...
/**
  ...
  */
__weak void STO_SetMinStartUpValidSpeedUnit(STO_PLL_Handle_t *pHandle, uint16_t hMinStartUpValidSpeed)  //设置最小机械速度的绝对值
{
#ifdef NULL_PTR_STO_PLL_SPD_POS_FDB
  if (MC_NULL == pHandle)
  {
...
/**
  ...
  */
__weak void STO_SetDirection(STO_PLL_Handle_t *pHandle, int8_t direction) //设置旋转方向
{
#ifdef NULL_PTR_STO_PLL_SPD_POS_FDB
  if (MC_NULL == pHandle)
  {
