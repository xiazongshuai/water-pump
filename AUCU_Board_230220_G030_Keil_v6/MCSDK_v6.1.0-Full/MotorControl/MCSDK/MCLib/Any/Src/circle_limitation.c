...
/**
  ...
  */
	...
	//为了使Vd，Vq合成的电压矢量小于等于单位圆的边，即Vd2 + Vq2 ≤ MAX_MODULE2。(平方)
__weak qd_t Circle_Limitation(const CircleLimitation_Handle_t *pHandle, qd_t Vqd)  //v_q, v_d限幅
{
  qd_t Local_Vqd = Vqd;
  ...
  else
  {
    ...
    maxModule = (int32_t)pHandle->MaxModule; //32767*0.97
    ...
    {
			watch_data.w_flag_circle_limit = 1;
      if (square_d <= vd_square_limit)
      {
#if defined CIRCLE_LIMITATION_SQRT_M0
        square_temp = (square_limit - square_d) / 1048576;  //2^20
