...
__weak void PID_HandleInit(PID_Handle_t *pHandle)
{
  ...
  else
  {
    ...
    pHandle->wPrevProcessVarError = 0;  //设置PID组件的积分项的值
#ifdef NULL_PTR_CHECK_PID_REG
  }
#endif
}
...
/**
  ...
  * integer value.
  */
__weak void PID_SetIntegralTerm(PID_Handle_t *pHandle, int32_t wIntegralTermValue) //设置积分项的值
{
#ifdef NULL_PTR_CHECK_PID_REG
  if (MC_NULL == pHandle)
  {
...
/**
  ...
  */
__weak uint16_t PID_GetKPDivisor(PID_Handle_t *pHandle)  //KP放大倍数
{
#ifdef NULL_PTR_CHECK_PID_REG
...
__weak void PID_SetKIDivisorPOW2(PID_Handle_t *pHandle, uint16_t hKiDivisorPOW2)
{
  ...
  else
  {
    ...
    PID_SetUpperIntegralTermLimit(pHandle, (int32_t)INT16_MAX * (int32_t)wKiDiv);      //积分限幅
...
/**
  ...
  * integer value.
  */
__weak void PID_SetLowerIntegralTermLimit(PID_Handle_t *pHandle, int32_t wLowerLimit)  //积分下限
{
#ifdef NULL_PTR_CHECK_PID_REG
  if (MC_NULL == pHandle)
  {
...
/**
  ...
  * integer value.
  */
__weak void PID_SetUpperIntegralTermLimit(PID_Handle_t *pHandle, int32_t wUpperLimit) //积分上限
{
#ifdef NULL_PTR_CHECK_PID_REG
  if (MC_NULL == pHandle)
  {
...
/**
  ...
  */
__weak void PID_SetLowerOutputLimit(PID_Handle_t *pHandle, int16_t hLowerLimit)  //设置输出下线
{
#ifdef NULL_PTR_CHECK_PID_REG
  if (MC_NULL == pHandle)
  {
...
/**
  ...
  */
__weak void PID_SetUpperOutputLimit(PID_Handle_t *pHandle, int16_t hUpperLimit) //设置积分上限
{
#ifdef NULL_PTR_CHECK_PID_REG
  if (MC_NULL == pHandle)
  {
...
/**
  ...
  */
__weak void PID_SetPrevError(PID_Handle_t *pHandle, int32_t wPrevProcessVarError)  //设置上一个误差
{
#ifdef NULL_PTR_CHECK_PID_REG
  if (MC_NULL == pHandle)
  {
...
__weak int16_t PI_Controller(PID_Handle_t *pHandle, int32_t wProcessVarError)
{
  ...
  else
  {
    ...
    wProportional_Term = pHandle->hKpGain * wProcessVarError;        //比例项的值

    /* Integral term computation */
    if (0 == pHandle->hKiGain)
    {
      pHandle->wIntegralTerm = 0;           //积分项的值
    }
    else
    {
      ...
      if (wIntegral_sum_temp < 0)
      {
        if (pHandle->wIntegralTerm > 0)
        {
          if (wIntegral_Term > 0)
          {
            wIntegral_sum_temp = INT32_MAX;  //两个大于0的数相加和小于0--->溢出---->取上限值
          }
          else
          {
            /* Nothing to do */
      ...
      else
      {
        if (pHandle->wIntegralTerm < 0)
        {
          if (wIntegral_Term < 0)
          {
            wIntegral_sum_temp = -INT32_MAX;  //取下限值
          }
          else
          {
            /* Nothing to do */
      ...
      if (wIntegral_sum_temp > pHandle->wUpperIntegralLimit)  //积分限幅（上限）
      {
        pHandle->wIntegralTerm = pHandle->wUpperIntegralLimit;
      }
      else if (wIntegral_sum_temp < pHandle->wLowerIntegralLimit) //积分限幅（下限）
      {
    ...
    wOutput_32 = (wProportional_Term >> pHandle->hKpDivisorPOW2) + (pHandle->wIntegralTerm >> pHandle->hKiDivisorPOW2);   //比例项的除数(右移的方式)
#else
    wOutput_32 = (wProportional_Term / (int32_t)pHandle->hKpDivisor)       //比例项的除数(右移的方式)
              + (pHandle->wIntegralTerm / (int32_t)pHandle->hKiDivisor);
#endif

    if (wOutput_32 > hUpperOutputLimit) //输出限幅
    {
    ...
    pHandle->wIntegralTerm += wDischarge;  //输出超出部分加在积分上
...
__weak int16_t PID_Controller(PID_Handle_t *pHandle, int32_t wProcessVarError)
{
  ...
  else
  {
	...
	if (0 == pHandle->hKdGain) /* derivative terms not used */     //微分系数为0执行PI控制
    {
