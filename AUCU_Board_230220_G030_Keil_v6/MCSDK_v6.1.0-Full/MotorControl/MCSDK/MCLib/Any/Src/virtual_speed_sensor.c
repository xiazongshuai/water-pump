...
/**
  ...
  */
__weak void VSS_Init(VirtualSpeedSensor_Handle_t *pHandle) //设置一个虚拟的速度传感器,系统设置一个虚构的速度传感器，读取电机的实际转速。
{
  VSS_Clear(pHandle);
}
...
__weak void VSS_Clear(VirtualSpeedSensor_Handle_t *pHandle)
{
  ...
  {
#endif
    pHandle->_Super.bSpeedErrorNumber = 0U; //平均机械速度无效的次数。
...
#endif
/**
  * @brief  Updates the rotor electrical angle integrating the last settled     //更新转子电角度，整合最后一次结算瞬时电速度
  ...
  */
__weak int16_t VSS_CalcElAngle(VirtualSpeedSensor_Handle_t *pHandle, int16_t *pInputVars_str)  //pInputVars_str传入的转子电角度
{
  int16_t hRetAngle;
#ifdef NULL_PTR_VIR_SPD_SEN
  ...
  else
  {
    ...
    {
      pHandle->hElAngleAccu += pHandle->_Super.hElSpeedDpp; //电角度累加
      pHandle->_Super.hMecAngle += (pHandle->_Super.hElSpeedDpp / (int16_t)pHandle->_Super.bElToMecRatio); //机械角度累加
      ...
      if (true == pHandle->bTransitionStarted)
      {
        ...
        {
          pHandle->hTransitionRemainingSteps--; //结束从虚拟速度传感器模块到其他速度传感器模块的转换的剩余步骤数
...
/**
  ...
  */
__weak bool VSS_CalcAvrgMecSpeedUnit(VirtualSpeedSensor_Handle_t *pHandle, int16_t *hMecSpeedUnit)  //加速度积分计算平均速度
{
  ...
  else
  {
    ...
    if (pHandle->hRemainingStep > 1u)
    {
      ...
      *hMecSpeedUnit = (int16_t)((((int32_t)pHandle->_Super.hElSpeedDpp)
      ...
      pHandle->_Super.hAvrMecSpeedUnit = *hMecSpeedUnit; //机械速度
    ...
    {
      SpeedSensorReliability = SPD_IsMecSpeedReliable(&pHandle->_Super, hMecSpeedUnit);  //速度、加速度是否在可靠范围内
    }
#ifdef NULL_PTR_VIR_SPD_SEN
  }
#endif
                                    ...
                                    uint16_t hDurationms)
{
  ...
  else
  {
    ...
    if (false == pHandle->bTransitionStarted)
    {
      if (0U == hDurationms)
      {
        pHandle->_Super.hAvrMecSpeedUnit = hFinalMecSpeedUnit;  //速度
         
				//hElSpeedDpp电角度  DPPConvFactor转换系数
        pHandle->_Super.hElSpeedDpp = (int16_t)((((int32_t)hFinalMecSpeedUnit)
                                               * ((int32_t)pHandle->_Super.DPPConvFactor))
                                              / (((int32_t)SPEED_UNIT)
                                               * ((int32_t)pHandle->_Super.hMeasurementFrequency)));  //机械速度单位转换
        //bElToMecRatio 电气量转为机械量的系数
        pHandle->_Super.hElSpeedDpp *= ((int16_t)pHandle->_Super.bElToMecRatio);  //机械速度转电气速度
      ...
      {
        hNbrStep = (uint16_t)((((uint32_t)hDurationms) * ((uint32_t)pHandle->hSpeedSamplingFreqHz)) / 1000U);   //分为多少步
        hNbrStep++;
        pHandle->hRemainingStep = hNbrStep;
        hCurrentMecSpeedDpp = pHandle->_Super.hElSpeedDpp / ((int16_t)pHandle->_Super.bElToMecRatio);   //电角度转机械角度
        hFinalMecSpeedDpp = (int16_t)((((int32_t )hFinalMecSpeedUnit) * ((int32_t)pHandle->_Super.DPPConvFactor))
                                    / (((int32_t )SPEED_UNIT) * ((int32_t)pHandle->_Super.hMeasurementFrequency)));  //机械速度单位转换 
        ...
        {
          wMecAccDppP32 = ((((int32_t)hFinalMecSpeedDpp) - ((int32_t)hCurrentMecSpeedDpp))  //每一步的机械角度量(加速度)
                         * ((int32_t)65536)) / ((int32_t )hNbrStep);

          pHandle->wElAccDppP32 = wMecAccDppP32 * ((int16_t)pHandle->_Super.bElToMecRatio);  //每一步的电角度量(加速度)
        }
