...
/* USER CODE END MC_Scheduler 0 */
  ...
  if (((uint8_t)1) == bMCBootCompleted) //初始化完成
  {
    ...
    {
      hBootCapDelayCounterM1--;  //充电时间
    }
    if(hStopPermanencyCounterM1 > 0U)
    {
      hStopPermanencyCounterM1--;  //停机状态的时间
    }
  }
  else
  {
...
__weak void TSK_MediumFrequencyTaskM1(void)  //中频任务
{
  ...
  int16_t wAux = 0;
  bool IsSpeedReliable = STO_PLL_CalcAvrgMecSpeedUnit(&STO_PLL_M1, &wAux); //计算平均机械速度，返回是否可靠
//	w_speed = wAux * 60 / 10;
	watch_data.w_speed = wAux * 60 / 10;
  PQD_CalcElMotorPower(pMPM[M1]); //计算功率

  if (MCI_GetCurrentFaults(&Mci[M1]) == MC_NO_FAULTS)
  {
    if (MCI_GetOccurredFaults(&Mci[M1]) == MC_NO_FAULTS) //没有故障
    {
      switch (Mci[M1].State)
      {
        case IDLE:
        {                                             //已经执行了ADC偏移测量程序   R3_1_CurrentReadingCalibration
          if ((MCI_START == Mci[M1].DirectCommand) || (MCI_MEASURE_OFFSETS == Mci[M1].DirectCommand))   //MC_StartMotor1 会设置该标志位 MCI_START
          {
            {
              RUC_Clear(&RevUpControlM1, MCI_GetImposedMotorDirection(&Mci[M1]));
            }

           if (pwmcHandle[M1]->offsetCalibStatus == false) //偏移校准未完成
           {
           ...
           else  //偏移校准完成
           {
             ...
             R3_1_TurnOnLowSides(pwmcHandle[M1],M1_CHARGE_BOOT_CAP_DUTY_CYCLES); //打开下桥
             TSK_SetChargeBootCapDelayM1(CHARGE_BOOT_CAP_TICKS); //设置时间驱动器启动电容器为电机充电
        ...
        case OFFSET_CALIB:
          {
            ...
            else
            {
              ...
              {
                if (MCI_MEASURE_OFFSETS == Mci[M1].DirectCommand)//启动ADC偏移测量程序
                {
                  FOC_Clear(M1);
                  PQD_Clear(pMPM[M1]);
        ...
        case START:
        {
          ...
          else
          {
           ...
           if (true == RUC_FirstAccelerationStageReached(&RevUpControlM1))  //第一加速阶段是否完成
            {
            ...
            if (ObserverConverged) ////状态观测器算法收敛。
            {
        ...
        case SWITCH_OVER:
        {
          ...
          else
          {
...

            {
              ...
              LoopClosed = VSS_CalcAvrgMecSpeedUnit(&VirtualSpeedSensorM1, &hForcedMecSpeedUnit); //计算转子的虚拟速度和位置。如果虚拟速度在可靠性范围内，则函数返回true
              /* Check if the transition ramp has completed. */
              bool tempBool;
              tempBool = VSS_TransitionEnded(&VirtualSpeedSensorM1); //斜坡是否完成
              ...
              if (true ==  LoopClosed)
              {
                ...
                PID_SetIntegralTerm(&PIDSpeedHandle_M1,
                                    ...
                                    / PID_SPEED_INTEGRAL_INIT_DIV));  //设置积分项的值
				#endif
                ...
                FOC_CalcCurrRef( M1 );
                STC_ForceSpeedReferenceToCurrentSpeed(pSTC[M1]); /* Init the reference speed to current speed */ //将参考速度初始化为当前速度
                MCI_ExecBufferedCommands(&Mci[M1]); /* Exec the speed ramp after changing of the speed sensor */ //更换速度传感器后执行速度斜坡
                Mci[M1].State = RUN;
              }
            }
          }
...
/**
  ...
  * @retval void
  */
__weak void TSK_SetChargeBootCapDelayM1(uint16_t hTickCount) //设置时间驱动器启动电容器为电机1充电
{
...
/**
  ...
  * @retval void
  */
__weak void TSK_SetStopPermanencyTimeM1(uint16_t hTickCount)  //处于停机状态的时间
{
...
/**
  ...
  */
__weak uint8_t TSK_HighFrequencyTask(void)  //高频任务
{
  ...
  {
    bool IsAccelerationStageReached = RUC_FirstAccelerationStageReached(&RevUpControlM1); //第一加速阶段是否完成
    STO_Inputs.Ialfa_beta = FOCVars[M1].Ialphabeta; /*  only if sensorless*/
    STO_Inputs.Vbus = VBS_GetAvBusVoltage_d(&(BusVoltageSensor_M1._Super)); /*  only for sensorless*/  //母线电压
    (void)( void )STO_PLL_CalcElAngle(&STO_PLL_M1, &STO_Inputs);    //反电动势计算电角度
    STO_PLL_CalcAvrgElSpeedDpp(&STO_PLL_M1); /*  Only in case of Sensor-less */  //计算电角度速度
...
inline uint16_t FOC_CurrControllerM1(void)
{
  ...
  RCM_ExecNextConv(); //ADC转换
  Ialphabeta = MCM_Clarke(Iab);
  Iqd = MCM_Park(Ialphabeta, hElAngle);
	
	//电流环
  Vqd.q = PI_Controller(pPIDIq[M1], (int32_t)(FOCVars[M1].Iqdref.q) - Iqd.q);  //FOCVars[M1].Iqdref.q  参考电流
  ...
  RCM_ReadOngoingConv();
  hCodeError = PWMC_SetPhaseVoltage(pwmcHandle[M1], Valphabeta); //设置每相的占空比
  ...
  FOCVars[M1].hElAngle = hElAngle;
		...
		watch_data.w_I_uv_digital.I_u_digital = Iab.a;
		watch_data.w_I_uv_digital.I_v_digital = Iab.b;   

		//watch_data.w_I_uv_physical.I_u_physical = ((int32_t)(Iab.a * watch_data.w_max_current_multi_1000))>>15;
		//watch_data.w_I_uv_physical.I_v_physical = ((int32_t)(Iab.b * watch_data.w_max_current_multi_1000))>>15;              
		watch_data.w_Iqd_act.Iq_act_digital = Iqd.q;
		//watch_data.w_Iqd_act.Id_act_digital = Iqd.d;
		watch_data.w_Iqd_ref.Iq_ref_digital = FOCVars[M1].Iqdref.q;
		//watch_data.w_Iqd_ref.Id_ref_digital = FOCVars[M1].Iqdref.d;

		watch_data.w_Iqd_act_phy.Iq_act_physical = ((int32_t)(Iqd.q * watch_data.w_max_current_multi_1000))>>15;
		watch_data.w_Iqd_ref_phy.Iq_ref_physical = ((int32_t)(FOCVars[M1].Iqdref.q * watch_data.w_max_current_multi_1000))>>15;
...
/**
  ...
  */
__weak void TSK_SafetyTask(void)  //安全任务
{
  /* USER CODE BEGIN TSK_SafetyTask 0 */

  /* USER CODE END TSK_SafetyTask 0 */
  if (1U == bMCBootCompleted) //初始化完成
  {
...
__weak void TSK_SafetyTask_PWMOFF(uint8_t bMotor)
{
  ...
  if (MCI_GetFaultState(&Mci[bMotor]) != (uint32_t)MC_NO_FAULTS)  //出现故障信号
  {
    PWMC_SwitchOffPWM(pwmcHandle[bMotor]);
