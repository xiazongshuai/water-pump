...
__weak void MCI_ExecBufferedCommands(MCI_Handle_t *pHandle)
{
  ...
  {
#endif
    if ( pHandle->CommandState == MCI_COMMAND_NOT_ALREADY_EXECUTED ) //缓冲命令尚未发生
    {
      bool commandHasBeenExecuted = false;
      switch (pHandle->lastCommand)
      {
        case MCI_CMD_EXECSPEEDRAMP: //执行速度斜坡
        {
          pHandle->pFOCVars->bDriveInput = INTERNAL;
          STC_SetControlMode(pHandle->pSTC, MCM_SPEED_MODE);
          commandHasBeenExecuted = STC_ExecRamp(pHandle->pSTC, pHandle->hFinalSpeed, pHandle->hDurationms); //执行斜坡
          break;
        }

        case MCI_CMD_EXECTORQUERAMP: //执行转矩斜坡
        {
          pHandle->pFOCVars->bDriveInput = INTERNAL;
        ...
        case MCI_CMD_SETCURRENTREFERENCES: //设置电流参考??
        {
          pHandle->pFOCVars->bDriveInput = EXTERNAL;
