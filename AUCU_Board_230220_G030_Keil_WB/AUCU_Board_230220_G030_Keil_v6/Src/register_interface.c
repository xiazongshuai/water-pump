...
static PID_Handle_t *pPIDSpeed[NBR_OF_MOTORS] = { &PIDSpeedHandle_M1 };  //速度环PID
static PID_Handle_t *pPIDFW[NBR_OF_MOTORS] = { &PIDFluxWeakeningHandle_M1};  //弱磁环PID
        ...
        *size = rawSize + 2U;
        ...
        else
        {
          switch (regID)
          {
            ...
            {
              int32_t rpm, delta_rpm;
              uint16_t duration;
							uint32_t	duration_2_fixedRamp;
...
//							if(rpm)
//							delta_rpm = exp(rpm - watch_data.w_hTargetSpeed_rpm);
//							duration_2_fixedRamp = delta_rpm * 10; //1rpm need 10ms, 1000rpm need 10s
//							if(duration_2_fixedRamp >= 65535 ) 
//								duration_2_fixedRamp = 65535;
