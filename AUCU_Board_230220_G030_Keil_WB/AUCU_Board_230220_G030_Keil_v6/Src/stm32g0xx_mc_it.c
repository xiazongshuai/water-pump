...
void USART2_IRQHandler(void)
{
  ...
  /* USER CODE BEGIN USART2_IRQHandler 1 */
    ...
    {
		mrx_data = LL_USART_ReceiveData8(USART2);
			#if 0
	   rx_tmp_data = LL_USART_ReceiveData8(USART2);
     if(rx_sta == 0)
		 {
	    if(rx_tmp_data == display_panel_id)
			{
			  rx_data[0] = rx_tmp_data;
//				LL_USART_TransmitData8(USART2, rx_data[0]);
				recvlen++;
				rx_sta = 1;
			}
		 }
		 ...
		 {
		   rx_data[recvlen] = rx_tmp_data;
//			 LL_USART_TransmitData8(USART2, rx_data[recvlen]);
			 recvlen++;
			 if(recvlen == 20)
			 {
				 recvlen = 0;
