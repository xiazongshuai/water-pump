...
//static volatile uint16_t hBootCapDelayCounterM1 = ((uint16_t)0);
 volatile uint16_t hBootCapDelayCounterM1 = ((uint16_t)0);
...
//    STC_GetMecSpeedRefUnitDefault(pSTC[M1]),0); /*First command to STC*/
    ...
    FW_Init(pFW[M1],&PIDSpeedHandle_M1,&PIDFluxWeakeningHandle_M1);
		...
		MCI_ExecSpeedRamp(&Mci[M1],0,0); /*First command to STC*/
    pMCIList[M1] = &Mci[M1];
...
/* USER CODE END MC_Scheduler 0 */
  ...
  if (((uint8_t)1) == bMCBootCompleted) //³õÊ¼»¯Íê³É
  {
    ...
    else //500us*2Ö´ÐÐÒ»´Î£¿£¿
    {
      ...
      hMFTaskCounterM1 = (uint16_t)MF_TASK_OCCURENCE_TICKS; //(2000/1000)-1 = 1
    }
    if(hBootCapDelayCounterM1 > 0U)
    {
      hBootCapDelayCounterM1--;  //³äµçÊ±¼ä
    }
    if(hStopPermanencyCounterM1 > 0U)
    {
      hStopPermanencyCounterM1--;  //Í£»ú×´Ì¬µÄÊ±¼ä
    }
  }
  else
  {
...
/**
  ...
  * are executed here.
  */
//int16_t	w_speed = 0;	//unit: rpm
__weak void TSK_MediumFrequencyTaskM1(void)  //ÖÐÆµÈÎÎñ
{
  ...
  int16_t wAux = 0;
  bool IsSpeedReliable = STO_PLL_CalcAvrgMecSpeedUnit(&STO_PLL_M1, &wAux); //¼ÆËãÆ½¾ù»úÐµËÙ¶È£¬·µ»ØÊÇ·ñ¿É¿¿
//	w_speed = wAux * 60 / 10;
	watch_data.w_speed = wAux * 60 / 10;
  PQD_CalcElMotorPower(pMPM[M1]); //¼ÆËã¹¦ÂÊ

  if (MCI_GetCurrentFaults(&Mci[M1]) == MC_NO_FAULTS)
  {
    if (MCI_GetOccurredFaults(&Mci[M1]) == MC_NO_FAULTS) //Ã»ÓÐ¹ÊÕÏ
    {
      switch (Mci[M1].State)
      {
        case IDLE:
        {                                             //ÒÑ¾­Ö´ÐÐÁËADCÆ«ÒÆ²âÁ¿³ÌÐò   R3_1_CurrentReadingCalibration
          if ((MCI_START == Mci[M1].DirectCommand) || (MCI_MEASURE_OFFSETS == Mci[M1].DirectCommand))   //MC_StartMotor1 »áÉèÖÃ¸Ã±êÖ¾Î» MCI_START
          {
            {
              RUC_Clear(&RevUpControlM1, MCI_GetImposedMotorDirection(&Mci[M1]));
            }

           if (pwmcHandle[M1]->offsetCalibStatus == false) //Æ«ÒÆÐ£×¼Î´Íê³É
           {
             PWMC_CurrentReadingCalibr(pwmcHandle[M1], CRC_START); //Í¨¹ý¶ÁÈ¡Ã»ÓÐµçÁ÷Ê±µÄµçÁ÷ÖµµçÑ¹Öµ×÷ÎªÐ£×¼£¬Ã¿´ÎÆô¶¯¶¼ÐèÒªµ÷ÓÃ¡£
           ...
           else  //Æ«ÒÆÐ£×¼Íê³É
           {
             ...
             R3_1_TurnOnLowSides(pwmcHandle[M1],M1_CHARGE_BOOT_CAP_DUTY_CYCLES); //´ò¿ªÏÂÇÅ
             TSK_SetChargeBootCapDelayM1(CHARGE_BOOT_CAP_TICKS); //ÉèÖÃÊ±¼äÇý¶¯Æ÷Æô¶¯µçÈÝÆ÷Îªµç»ú³äµç
        ...
        case OFFSET_CALIB:
          {
            ...
            else
            {
              ...
              {
                if (MCI_MEASURE_OFFSETS == Mci[M1].DirectCommand)//Æô¶¯ADCÆ«ÒÆ²âÁ¿³ÌÐò
                {
                  FOC_Clear(M1);
                  PQD_Clear(pMPM[M1]);
        ...
        case START:
        {
          ...
          else
          {
           ...
           if (true == RUC_FirstAccelerationStageReached(&RevUpControlM1))  //µÚÒ»¼ÓËÙ½×¶ÎÊÇ·ñÍê³É
            ...
            if (ObserverConverged) ////×´Ì¬¹Û²âÆ÷Ëã·¨ÊÕÁ²¡£
            {
              qd_t StatorCurrent = MCM_Park(FOCVars[M1].Ialphabeta, SPD_GetElAngle(&STO_PLL_M1._Super));

              /* Start switch over ramp. This ramp will transition from the revup to the closed loop FOC. */
              REMNG_Init(pREMNG[M1]);
              (void)REMNG_ExecRamp(pREMNG[M1], FOCVars[M1].Iqdref.q, 0);  //¼ÆËãÐ±ÆÂ²½Êý£¬Ã¿²½¼Ó¼õÁ¿
        ...
        case SWITCH_OVER:
        {
          ...
          else
          {
...

            {
              ...
              LoopClosed = VSS_CalcAvrgMecSpeedUnit(&VirtualSpeedSensorM1, &hForcedMecSpeedUnit); //¼ÆËã×ª×ÓµÄÐéÄâËÙ¶ÈºÍÎ»ÖÃ¡£Èç¹ûÐéÄâËÙ¶ÈÔÚ¿É¿¿ÐÔ·¶Î§ÄÚ£¬Ôòº¯Êý·µ»Øtrue
              /* Check if the transition ramp has completed. */
              bool tempBool;
              tempBool = VSS_TransitionEnded(&VirtualSpeedSensorM1); //Ð±ÆÂÊÇ·ñÍê³É
              ...
              if (true ==  LoopClosed)
              {
                ...
                PID_SetIntegralTerm(&PIDSpeedHandle_M1,
                                    ...
                                    / PID_SPEED_INTEGRAL_INIT_DIV));  //ÉèÖÃ»ý·ÖÏîµÄÖµ
				#endif
                ...
                FOC_CalcCurrRef( M1 );
                STC_ForceSpeedReferenceToCurrentSpeed(pSTC[M1]); /* Init the reference speed to current speed */ //½«²Î¿¼ËÙ¶È³õÊ¼»¯Îªµ±Ç°ËÙ¶È
                MCI_ExecBufferedCommands(&Mci[M1]); /* Exec the speed ramp after changing of the speed sensor */ //¸ü»»ËÙ¶È´«¸ÐÆ÷ºóÖ´ÐÐËÙ¶ÈÐ±ÆÂ
                Mci[M1].State = RUN;
              }
            }
          }
        ...
        case RUN:
        {
          ...
          else
          {
            ...
            MCI_ExecBufferedCommands(&Mci[M1]);
              ...
              FOC_CalcCurrRef(M1);  //ÓÉËÙ¶È¼ÆËãµçÁ÷
...
/**
  ...
  * @retval void
  */
__weak void TSK_SetChargeBootCapDelayM1(uint16_t hTickCount) //ÉèÖÃÊ±¼äÇý¶¯Æ÷Æô¶¯µçÈÝÆ÷Îªµç»ú1³äµç
{
...
/**
  ...
  * @retval void
  */
__weak void TSK_SetStopPermanencyTimeM1(uint16_t hTickCount)  //´¦ÓÚÍ£»ú×´Ì¬µÄÊ±¼ä
{
...
/**
  ...
  */
__weak uint8_t TSK_HighFrequencyTask(void)  //¸ßÆµÈÎÎñ
{
  ...
  {
    bool IsAccelerationStageReached = RUC_FirstAccelerationStageReached(&RevUpControlM1); //µÚÒ»¼ÓËÙ½×¶ÎÊÇ·ñÍê³É
    STO_Inputs.Ialfa_beta = FOCVars[M1].Ialphabeta; /*  only if sensorless*/
    STO_Inputs.Vbus = VBS_GetAvBusVoltage_d(&(BusVoltageSensor_M1._Super)); /*  only for sensorless*/  //Ä¸ÏßµçÑ¹
    (void)( void )STO_PLL_CalcElAngle(&STO_PLL_M1, &STO_Inputs);    //·´µç¶¯ÊÆ¼ÆËãµç½Ç¶È
    STO_PLL_CalcAvrgElSpeedDpp(&STO_PLL_M1); /*  Only in case of Sensor-less */  //¼ÆËãµç½Ç¶ÈËÙ¶È
...
/**
  ...
  */
//ab_t 	w_Iab;				//w_Iab_digital;
//float w_Iab_physical;
int cbv = 0;
int test_table[4];
inline uint16_t FOC_CurrControllerM1(void)
{
  ...
  PWMC_GetPhaseCurrents(pwmcHandle[M1], &Iab); //R3_1_GetPhaseCurrents  Ia/Ib/Ic

  RCM_ExecNextConv(); //ADC×ª»»
  Ialphabeta = MCM_Clarke(Iab);
  Iqd = MCM_Park(Ialphabeta, hElAngle);
	
	//µçÁ÷»·   Iqdref»á¸ù¾ÝËÙ¶È±ä»»
  Vqd.q = PI_Controller(pPIDIq[M1], (int32_t)(FOCVars[M1].Iqdref.q) - Iqd.q);  //FOCVars[M1].Iqdref.q  ²Î¿¼µçÁ÷
  Vqd.d = PI_Controller(pPIDId[M1], (int32_t)(FOCVars[M1].Iqdref.d) - Iqd.d);
	 

  Vqd = Circle_Limitation(&CircleLimitationM1, Vqd); //é™åˆ¶åœ¨åœ†å†…
  hElAngle += SPD_GetInstElSpeedDpp(speedHandle)*REV_PARK_ANGLE_COMPENSATION_FACTOR;
  Valphabeta = MCM_Rev_Park(Vqd, hElAngle);
  RCM_ReadOngoingConv();
  hCodeError = PWMC_SetPhaseVoltage(pwmcHandle[M1], Valphabeta); //ÉèÖÃÃ¿ÏàµÄÕ¼¿Õ±È
  ...
  FOCVars[M1].hElAngle = hElAngle;
	...
	watch_data.w_Vqd.Vd = Vqd.d;
	watch_data.w_Iab_digital.I_a = Iab.a;
	watch_data.w_Iab_digital.I_b = Iab.b;	
//w_Iab_physical = (w_Iab_digital/65535)*3.3*1.1/0.3 //AMPLIFICATION_GAIN: 1.1
//	watch_data.w_Iab_physical.a = (float)Iab.a * 12.1 / 65535.0;	
//	watch_data.w_Iab_physical.b = Iab.b * 0.121 / 65535;	
//	watch_data.w_Iab_physical.b = (float)Iab.b * 12.1 / 65535.0;

	watch_data.w_Iab_physical.a = ((int32_t)Iab.a * 12100 )>>16;	//I_a_physical * 1000, unit:A
	watch_data.w_Iab_physical.b = ((int32_t)Iab.b * 12100 )>>16;	//I_b_physical * 1000, unit:A	
	watch_data.w_Iqd_act.Iq_act = Iqd.q;
	watch_data.w_Iqd_act.Id_act = Iqd.d;
	watch_data.w_Iqd_ref.Iq_ref = FOCVars[M1].Iqdref.q;
	watch_data.w_Iqd_ref.Id_ref = FOCVars[M1].Iqdref.d;
	
	watch_data.w_hElAngle = hElAngle;
#endif	
  return(hCodeError);
}
...
/**
  ...
  */
__weak void TSK_SafetyTask(void)  //°²È«ÈÎÎñ
{
  /* USER CODE BEGIN TSK_SafetyTask 0 */

  /* USER CODE END TSK_SafetyTask 0 */
  if (1U == bMCBootCompleted) //³õÊ¼»¯Íê³É
  {
...
__weak void TSK_SafetyTask_PWMOFF(uint8_t bMotor)
{
  ...
  if (MCI_GetFaultState(&Mci[bMotor]) != (uint32_t)MC_NO_FAULTS)  //³öÏÖ¹ÊÕÏÐÅºÅ
  {
    PWMC_SwitchOffPWM(pwmcHandle[bMotor]);
