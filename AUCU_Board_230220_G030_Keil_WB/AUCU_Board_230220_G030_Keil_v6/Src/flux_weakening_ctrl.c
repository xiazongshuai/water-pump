...
__weak void FW_Init(FW_Handle_t *pHandle, PID_Handle_t *pPIDSpeed, PID_Handle_t *pPIDFluxWeakeningHandle)
{
  ...
  {
#endif
    pHandle->hFW_V_Ref = pHandle->hDefaultFW_V_Ref;   //98.5  �ο���ѹ��
...
__weak void FW_Clear(FW_Handle_t *pHandle)
{
  ...
  {
#endif
    PID_SetIntegralTerm(pHandle->pFluxWeakeningPID, (int32_t)0); //设置积分项的值
    pHandle->AvVolt_qd = V_null;
    pHandle->AvVoltAmpl = (int16_t)0; //定子平均电压幅值
...
__weak qd_t FW_CalcCurrRef(FW_Handle_t *pHandle, qd_t Iqdref)
{
  ...
  else
  {
    ...
    wVoltLimit_Ref = ((uint32_t)(pHandle->hFW_V_Ref) * pHandle->hMaxModule) / 1000U;  //最大值的百分比
    Aux2 = MCM_Modulus( pHandle->AvVolt_qd.q, pHandle->AvVolt_qd.d ); //根号下q^2+d^2
    pHandle->AvVoltAmpl = Aux2;  //平均电压幅值
    ...
    PID_SetLowerIntegralTermLimit(pHandle->pSpeedPID, -wAux1);  //积分限幅
...
#else
  return (pHandle->AvVoltAmpl); //返回实际使用的电压
#endif
}
...
#else
  return ((uint16_t)((uint32_t)(pHandle->AvVoltAmpl) * 1000U / (uint32_t)(pHandle->hMaxModule))); //hMaxModule ???
#endif
}
