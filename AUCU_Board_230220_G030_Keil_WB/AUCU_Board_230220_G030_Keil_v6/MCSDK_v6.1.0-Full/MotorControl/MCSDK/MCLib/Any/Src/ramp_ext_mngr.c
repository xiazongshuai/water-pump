...
/**
  ...
  */
__weak int32_t REMNG_Calc(RampExtMngr_Handle_t *pHandle)  //执行斜坡计算
{
  int32_t ret_val;
#ifdef NULL_RMP_EXT_MNG
  if (MC_NULL == pHandle)
...
/**
  ...
  */
__weak bool REMNG_ExecRamp(RampExtMngr_Handle_t *pHandle, int32_t TargetFinal, uint32_t Durationms)  //设置要执行的斜坡
{
  bool retVal = true;
#ifdef NULL_RMP_EXT_MNG
  if (MC_NULL == pHandle)
  ...
  else
  {
    ...
    else
    {
      ...
      aux /= 1000U;
      pHandle->RampRemainingStep = aux;  //完成斜坡的剩余步骤数。
      pHandle->RampRemainingStep++;
      ...
      pHandle->IncDecAmount = aux1;   //每步的加减量  
    }
#ifdef NULL_RMP_EXT_MNG
  }
#endif
...
/**
  ...
  */
__weak bool REMNG_RampCompleted(const RampExtMngr_Handle_t *pHandle)  //检查斜坡是否完成
{
  bool retVal = false;
#ifdef NULL_RMP_EXT_MNG
  if (MC_NULL == pHandle)
...
/**
  ...
  * @retval none
  */
__weak void REMNG_StopRamp(RampExtMngr_Handle_t *pHandle)  //停止执行斜坡
{
#ifdef NULL_RMP_EXT_MNG
  if (MC_NULL == pHandle)
  {
...
__weak uint32_t getScalingFactor(int32_t Target)
{
  ...
  for (i = 1U; i < 32U; i++)
  {
    uint32_t limit = (((uint32_t)1) << (31U - i));  //执行2^int（31-log2（Target））
    if (TargetAbs >= limit)
    {
      break;
    }
