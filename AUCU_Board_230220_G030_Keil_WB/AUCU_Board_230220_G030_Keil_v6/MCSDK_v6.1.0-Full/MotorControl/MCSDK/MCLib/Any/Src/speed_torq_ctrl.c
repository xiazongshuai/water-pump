...
//    int16_t hCurrentReference;
    ...
    if (MCM_TORQUE_MODE == pHandle->Mode)  //转矩模式
    {
      ...
      if ((int32_t)hTargetFinal > (int32_t)pHandle->MaxPositiveTorque)  //电机扭矩的最大正值。该值实际上表示以数字表示的最大Iq电流   电流限制（启动设定）
      {
        allowedRange = false;
      }
...
__weak int16_t STC_CalcTorqueReference(SpeednTorqCtrl_Handle_t *pHandle) //计算新的参考转矩
{
  ...
  else
  {
    ...
    else if (1U == pHandle->RampRemainingStep)
    {
      ...
      wCurrentReference = ((int32_t)pHandle->TargetFinal) * 65536;   //转矩或速度*65536
      pHandle->RampRemainingStep = 0U;
    }
    else
    {
    ...
    if (MCM_SPEED_MODE == pHandle->Mode)  //速度闭环
    {
      ...
      hTorqueReference = PI_Controller(pHandle->PISpeed, (int32_t)hError*6);  //PI控制
    ...
    {
      pHandle->TorqueRef = wCurrentReference; //转矩*65536
#ifndef FULL_MISRA_C_COMPLIANCY_SPD_TORQ_CTRL
      //cstat !MISRAC2012-Rule-1.3_n !ATH-shift-neg !MISRAC2012-Rule-10.1_R6
      hTorqueReference = (int16_t)(wCurrentReference >> 16); //实际设定转矩参考Iq
#else
