...
__weak void R3_1_Init(PWMC_R3_1_Handle_t *pHandle)
{
  ...
  if ((uint32_t)pHandle == (uint32_t)&pHandle->_Super)
  {
    ...
    LL_DMA_SetMemoryAddress(DMA1, LL_DMA_CHANNEL_1, (uint32_t)pHandle->ADC1_DMA_converted); //ADC转换数据
    LL_DMA_SetPeriphAddress( DMA1, LL_DMA_CHANNEL_1, ( uint32_t )&ADC1->DR );
    LL_DMA_SetDataLength(DMA1, LL_DMA_CHANNEL_1, 3);  //3个数据
    
    /* Enables the DMA1 Channel1 peripheral */
    LL_DMA_EnableChannel(DMA1, LL_DMA_CHANNEL_1);

    /* set default triggering edge */
    pHandle->ADCTriggerEdge = LL_ADC_REG_TRIG_EXT_RISING;  //上升沿触发
...
/*
  ...
  *         motor.
  * 
  *///当没有电流流入电机时，将Ia和Ib电流反馈模拟通道上的电压存储到@p pHdl处理器中。
...
#pragma cstat_restore = "MISRAC2012-Rule-11.3"
  ...
  case SECTOR_5: 
    ...
    wAux = (int32_t)(pHandle->PhaseAOffset)-(int32_t)(hReg1);  //a
	  if(abs(wAux) < 5 && target_p_s>1000)
		{
		 // printf("xx2\r\n");
			testhReg1 = abs(wAux);
    ...
    wAux = (int32_t)(pHandle->PhaseBOffset)-(int32_t)(hReg2);  //b
    if(abs(wAux) < 5 && target_p_s>1000)
		{
		  //printf("xx1\r\n");
			testhReg2 = abs(wAux);
...
//		if(pStator_Currents->a == 0)
//		{
//		  pStator_Currents->a = -(pStator_Currents->b/2);
//		}
//		if(pStator_Currents->b == 0)
//		{
//		  pStator_Currents->b = -(pStator_Currents->a/2);
//		}
    break;
  ...
  case SECTOR_1:  
    ...
    /* Ib = (PhaseBOffset - ADC converted value) ------------------------------*/ 
    wAux = (int32_t)(pHandle->PhaseBOffset)-(int32_t)(hReg1);  //b
    if(abs(wAux) < 10 &&target_p_s>1000)
		{
		  //printf("x1\r\n");
			testhReg1 = abs(wAux);
			pStator_Currents->b = 0;
   ...
   /* Saturation of Ib */
    ...
    wAux = (int32_t)(pHandle->PhaseCOffset)-(int32_t)(hReg2); //c
		if(abs(wAux) < 10&&target_p_s>1000)
		{
		  //printf("x6\r\n");
			testhReg2 = abs(wAux);
...
//		if(pStator_Currents->b == 0)
//		{
//		  pStator_Currents->a = (pStator_Currents->a/2); //a=-c
//		  pStator_Currents->b = (pStator_Currents->a/2);
//		}
//		if(test_c == 0)
//		{
//		  pStator_Currents->a = (pStator_Currents->a/2); //a=-b
...
//		{
//		  pStator_Currents->a = (pStator_Currents->b/2); 
//		  pStator_Currents->b = (pStator_Currents->b/2);
//		}
//		if(test_c == 0)
//		{
...
//	 if(pStator_Currents->a == 0 || pStator_Currents->b == 0 ) //下桥关闭
//	{
//	  printf("collect ia ib is zero\r\n");
//	}
}
...
#pragma cstat_restore = "MISRAC2012-Rule-11.3"
  ...
  /* Check if sampling AB in the middle of PWM is possible */  //检查是否可以在PWM中间采样AB
  ...
  {                           //2000                                    死区加上升噪声
    ...
    /* set sampling  point trigger in the middle of PWM period */  //在PWM周期中间设置采样点触发器
  ...
  else
  {
    ...
    {
      hCntSmp = lowDuty + pHandle->pParams_str->hTafter; //死区加上升噪声
			
      if ( hCntSmp >= pHandle->Half_PWMPeriod )  //超过最大值
      {
        pHandle->ADCTriggerEdge = LL_ADC_REG_TRIG_EXT_FALLING;   //下降沿触发
  ...
  return R3_1_WriteTIMRegisters( &pHandle->_Super, hCntSmp );  //设置占空比 采样点
}
...
#pragma cstat_restore = "MISRAC2012-Rule-11.3"
  ...
  LL_TIM_OC_SetCompareCH1 ( TIMx, (uint32_t)pHandle->_Super.CntPhA );  //设置占空比
  LL_TIM_OC_SetCompareCH2 ( TIMx, (uint32_t)pHandle->_Super.CntPhB );
  LL_TIM_OC_SetCompareCH3 ( TIMx, (uint32_t)pHandle->_Super.CntPhC );

//	LL_TIM_OC_SetCompareCH1 ( TIMx, (uint32_t)PWM_PERIOD_CYCLES/4 );  //设置占空比  占空比固定不能转动--锁住
//  LL_TIM_OC_SetCompareCH2 ( TIMx, (uint32_t)PWM_PERIOD_CYCLES/3 );
//  LL_TIM_OC_SetCompareCH3 ( TIMx, (uint32_t)PWM_PERIOD_CYCLES/2 );
	
  LL_TIM_OC_SetCompareCH4 ( TIMx, (uint32_t)hCCR4Reg );  //设置采样点
...
#pragma cstat_restore = "MISRAC2012-Rule-11.3"
  ...
  LL_DMA_EnableIT_TC(DMA1, LL_DMA_CHANNEL_1);  //ADC转换完成触发DMA中断

  /* Clear Update Flag */
  LL_TIM_ClearFlag_UPDATE( TIMx );
  /* Enable Update IRQ */
  LL_TIM_EnableIT_UPDATE( TIMx ); //更新中断
...
/*
  ...
  */
void * R3_1_TIMx_UP_IRQHandler( PWMC_R3_1_Handle_t * pHandle )  //下次ADC采样的触发方式、通道、方向
{
